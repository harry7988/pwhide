...
- **Stream forwarding (pipes)**: `cat data.sql | pwhide exec -- mysql -u {{db.user}} -p{{db}} ...` forwards stdin to the child byte-for-byte; output is binary-safe (`pwhide exec -- gzip -c f > out.gz` works) and still passes through the redactor. `cat deploy.sh | pwhide exec -f - --shell bash` reads the script from a pipe. With a piped stdin, the master passphrase must come from the keychain/env/file - pwhide fails fast with guidance instead of eating your data.
