...
- **stdin pipe passthrough boundary (0.10.0)**: in command mode pwhide forwards its stdin byte-for-byte to the child (stream forwarding). If redaction rules are inactive (the command references no secret placeholder and uses no --env), content the user pipes in — even if it happens to contain a password — reaches the child and output verbatim: pwhide cannot know what is in the user's pipe; redaction matches against vault secret values. When rules are active, piped data is scanned byte-level as well (covered by tests). Do not pipe passwords into pwhide exec — the injection path for secrets is placeholders.
