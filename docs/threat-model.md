...
- **stdin 管道直通边界（0.10.0）**：命令模式下 pwhide 的 stdin 逐字节转发给子进程（文件流转发特性）。若脱敏规则未激活（命令既不引用密文占位符也无 --env 注入），用户经管道传入的内容——即使恰好包含密码——会原样到达子进程与输出：pwhide 无从知晓用户管道里是什么，脱敏按库内密文值匹配。规则激活时管道数据同样被字节级扫描（实测覆盖）。不要把密码管道给 pwhide exec——密码的注入路径是占位符。
